...
--   players.position_group  <- PLAYERS.csv `Position` (backend department/grouping)
--   players.position_detail <- PLAYERS.csv `POS`      (frontend position display)
-- The live 902-player population was synchronized to this contract before this migration was committed.

drop function if exists public.filtered_player_leaderboard(text,text);
create function public.filtered_player_leaderboard(p_filter text default 'All',p_venue text default 'All')

alter table public.manager_spells
  add column if not exists profile_image_url_override text;

create or replace function public.filtered_manager_spell_leaderboard(p_filter text default 'All', p_venue text default 'All')
returns table(manager_spell_id bigint, manager_id bigint, sequence_order bigint, manager_order bigint, caretaker_label text, manager text, status text, declared_nation text, profile_image_url text, played bigint, won bigint, drawn bigint, lost bigint, goals_for bigint, goals_against bigint, goal_diff bigint, win_pct numeric, loss_pct numeric, first_match date, last_match date)
language sql
stable
as $function$
with spell_order as (
  select
    ...
    m.declared_nation,
    coalesce(ms.profile_image_url_override, m.profile_image_url) as profile_image_url
  from public.manager_spells ms
  join public.managers m on m.manager_id = ms.manager_id
),
selected as (
  select so.*, mt.match_id, mt.match_date, mt.result, mt.leeds_score, mt.opponent_score
  from spell_order so
...
select s.manager_spell_id, s.manager_id, s.sequence_order, s.manager_order, s.caretaker_label, s.manager, s.status, s.declared_nation, s.profile_image_url,
  count(s.match_id), count(*) filter(where s.result = 'Won'), count(*) filter(where s.result = 'Draw'), count(*) filter(where s.result = 'Lost'),
  coalesce(sum(s.leeds_score),0), coalesce(sum(s.opponent_score),0), coalesce(sum(s.leeds_score),0)-coalesce(sum(s.opponent_score),0),
  round(100.0*count(*) filter(where s.result='Won')/nullif(count(s.match_id),0),1),
  round(100.0*count(*) filter(where s.result='Lost')/nullif(count(s.match_id),0),1), min(s.match_date), max(s.match_date)
from selected s
group by s.manager_spell_id,s.manager_id,s.sequence_order,s.manager_order,s.caretaker_label,s.manager,s.status,s.declared_nation,s.profile_image_url

...
with league_tiers as (
 ...
 select a.*,
  ...
  round(94*percent_rank() over(order by players_per_season))::int personnel_score,
  round(94*percent_rank() over(order by days_in_charge))::int longevity_score
 from adj a
)
select manager_id,attacking_score,results_score,defensive_score,achievements_score,discipline_score,personnel_score,longevity_score,
 round(gf/nullif(played,0),2),round(100*wins/nullif(played,0),1),round(ga/nullif(played,0),2),round(clean_sheets/nullif(played,0),3),
 honours,promotions,relegations,red_cards,round(red_cards::numeric/nullif(played,0),4),players_used,seasons,round(players_per_season,2),days_in_charge,top_flight_titles
from ranked where manager_id=p_manager_id;
$function$;

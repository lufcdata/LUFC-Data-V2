...
type Page='matches'|'match-centre'|'goals'|'stat-pack'|'players'|'player-profile'|'managers'|'manager-profile'|'opponents'|'test-a1'|'player-admin';
function App() {
 ...
 const content=page==='matches'?<Matches onSelectMatch={openMatch}/>:page==='match-centre'?<MatchCentre matchId={selectedMatchId} onBack={()=>setPage('matches')}/>:page==='goals'?<Goals/>:page==='stat-pack'?<StatPack/>:page==='players'?<Players onSelectPlayer={openPlayer}/>:page==='player-profile'?<Fragment key={selectedPlayerId}><PlayerPage playerId={selectedPlayerId} onBack={()=>setPage('players')}/><PlayerMatchLog playerId={selectedPlayerId} playerName={selectedPlayerName}/></Fragment>:page==='managers'?<Managers onSelectManager={openManager}/>:page==='manager-profile'?<ManagerPage managerId={selectedManagerId} onBack={()=>setPage('managers')}/>:page==='test-a1'?<TestA1/>:page==='player-admin'?<PlayerAdmin/>:<Leaderboard/>;

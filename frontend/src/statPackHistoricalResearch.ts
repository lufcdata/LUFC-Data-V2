...
export function researchHistoricalFixtureContext(matches:FixtureResearchMatch[],fixture:UpcomingFixtureContext):FixtureResearchFinding[]{
 ...
 if(last&&won(last)){
  ...
  if(lastMargin>=2&&priorAtLeast){
   ...
   if(years>=5)add('Opponent History · Winning Margin',`Leeds' last meeting with ${fixture.opponent} produced their biggest winning margin against them since a ${priorAtLeast.leeds_score}-${priorAtLeast.opponent_score} victory in ${priorAtLeast.match_date.slice(0,7)}.`,98,`${versus.length} competitive meetings checked; previous equal-or-greater Leeds winning margin was match ${priorAtLeast.match_id}`,'opponent-winning-margin');
  }else if(lastMargin>=3&&previous.every(m=>margin(m)<lastMargin))add('Opponent History · Record Winning Margin',`Leeds' last meeting with ${fixture.opponent} produced their biggest recorded winning margin against them.`,100,`${versus.length} competitive meetings checked`,'opponent-winning-margin');
 }

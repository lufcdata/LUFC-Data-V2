...
type RadarRow={manager_id:number;red_cards:number};
type MatchRow={season:string|null};

export default function ManagerCareerMetrics({managerId}:{managerId:number}){
 const[metrics,setMetrics]=useState<Metric[]>([]);
 useEffect(()=>{let cancelled=false;(async()=>{if(!supabase)return;const[{data:leaderboard,error:lbError},{data:extra,error:extraError},{data:radar,error:radarError},{data:matches,error:matchError}]=await Promise.all([
  ...
  supabase.rpc('get_manager_performance_radar',{p_manager_id:managerId}),
  supabase.from('match_centre_summary').select('season').eq('leeds_manager_id',managerId)
 ]);if(lbError||extraError||radarError||matchError||cancelled)return;const current=((leaderboard??[])as LeaderboardRow[]).find(r=>Number(r.manager_id)===managerId),x=((extra??[])as MetricsRow[])[0],r=((radar??[])as RadarRow[])[0];if(!current||!x||!r){setMetrics([]);return}const seasons=new Set(((matches??[])as MatchRow[]).map(m=>m.season).filter((v):v is string=>Boolean(v))).size,p=Number(current.played),won=Number(current.won),drawn=Number(current.drawn),lost=Number(current.lost),gf=Number(current.goals_for),ga=Number(current.goals_against),gd=Number(current.goal_diff),winPct=Number(current.win_pct),oppPct=Number(x.opponents_defeated_pct),reds=Number(r.red_cards);const next:Metric[]=[
  ...
  {label:'Losses',value:String(lost),fill:p?lost/p*100:0},
  ...
  {label:'Red Cards',value:String(reds),fill:p?Math.min(100,reds/p*500):0,tone:'danger'},

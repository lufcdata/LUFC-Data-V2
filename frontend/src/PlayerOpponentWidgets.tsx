...
export default function PlayerOpponentWidgets({playerId}:{playerId:number}){
 ...
 useEffect(()=>{let cancelled=false;(async()=>{if(!supabase){setLoading(false);return}setLoading(true);setError(null);
  ...
  const[{data:clubData,error:clubError},{data:goalData,error:goalError}]=await Promise.all([
  ...
  if(clubError||goalError){if(!cancelled){setError((clubError??goalError)?.message??'Unable to load opposition record');setLoading(false)}return}

...
type Opponent = { club_id:number; opponent:string; crest_url:string|null; country:string|null; founded_year:number|null; nickname:string|null; played:number; won:number; drawn:number; lost:number; goals_for:number; goals_against:number; goal_diff:number; goals_per_game:number|string; clean_sheets:number; win_pct:number|string; loss_pct:number|string; last_meeting:string|null; last_won:string|null; last_lost:string|null; last5:string|null; };
type SortKey='country'|'founded_year'|'nickname'|'played'|'won'|'drawn'|'lost'|'points'|'goals_for'|'goals_against'|'goal_diff'|'goals_per_game'|'clean_sheets'|'win_pct'|'loss_pct'|'last_meeting'|'last_won'|'last_lost';
...
function compareText(a:string|null,b:string|null,dir:SortDir){if(!a&&!b)return 0;if(!a)return 1;if(!b)return-1;const c=a.localeCompare(b,undefined,{numeric:true,sensitivity:'base'});return dir==='asc'?c:-c;}

export default function Leaderboard({onSelectOpponent}:{onSelectOpponent?:(clubId:number)=>void}){
 ...
 const [country,setCountry]=useState('All Countries'); const [fivePlus,setFivePlus]=useState(false); const [search,setSearch]=useState('');
 useEffect(()=>{if(!supabase){setLoading(false);return;} setLoading(true);setError(null); supabase.rpc('filtered_opponent_leaderboard_v2',{p_filter:filter,p_venue:venue}).then(({data,error})=>{if(error)setError(error.message);else setTeams((data??[]) as Opponent[]);setLoading(false);}).catch((err:unknown)=>{setError(err instanceof Error?err.message:String(err));setLoading(false);});},[filter,venue]);
 useEffect(()=>{if(filter!=='Premier League'&&sortKey==='points'){setSortKey('played');setSortDir('desc');}},[filter,sortKey]);
 const countries=useMemo(()=>[...new Set(teams.map(t=>t.country).filter((v):v is string=>Boolean(v)))].sort((a,b)=>a.localeCompare(b)),[teams]);
 useEffect(()=>{if(country!=='All Countries'&&!countries.includes(country))setCountry('All Countries');},[countries,country]);
 const visible=useMemo(()=>{const q=search.trim().toLowerCase();return teams.filter(team=>(country==='All Countries'||team.country===country)&&(!fivePlus||Number(team.played)>=5)&&(!q||team.opponent.toLowerCase().includes(q)||(team.nickname??'').toLowerCase().includes(q)||(team.country??'').toLowerCase().includes(q)));},[teams,country,fivePlus,search]);
 ...
 const sorted=useMemo(()=>[...visible].sort((a,b)=>{
   if(sortKey==='country'||sortKey==='nickname'){const c=compareText(a[sortKey],b[sortKey],sortDir);return c||a.opponent.localeCompare(b.opponent);}
   if(sortKey==='founded_year'){const av=a.founded_year,bv=b.founded_year;if(av==null&&bv==null)return a.opponent.localeCompare(b.opponent);if(av==null)return 1;if(bv==null)return-1;const delta=av-bv;return delta===0?a.opponent.localeCompare(b.opponent):(sortDir==='asc'?delta:-delta);}
 ...
 function toggleSort(key:SortKey){if(key===sortKey)setSortDir(d=>d==='asc'?'desc':'asc');else{setSortKey(key);setSortDir(key==='country'||key==='nickname'||key==='founded_year'?'asc':'desc');}}
 ...
 <div className="card lb-table-card">
  ...
  <div className="lb-filterbar">
   ...
   <div className="lb-filter-section"><span className="lb-filter-label">Country</span><select className="lb-filter-select" value={country} onChange={e=>setCountry(e.target.value)}><option>All Countries</option>{countries.map(item=><option key={item}>{item}</option>)}</select></div>

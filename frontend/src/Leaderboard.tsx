...
type Opponent = {
  ...
  goals_for: number; goals_against: number; goal_diff: number; win_pct: number | string; last5: string; last_meeting: string;
};
type SortKey = 'played'|'won'|'drawn'|'lost'|'goals_for'|'goals_against'|'goal_diff'|'win_pct';
type SortDir = 'asc'|'desc';
const initials=(name:string)=>{const words=name.split(/\s+/).filter(Boolean);return (words.length===1?words[0].slice(0,3):words.slice(0,3).map(w=>w[0]).join('')).toUpperCase();};
...
export default function Leaderboard(){
 ...
 const max=useMemo(()=>({gf:Math.max(1,...teams.map(t=>t.goals_for)),ga:Math.max(1,...teams.map(t=>t.goals_against)),gd:Math.max(1,...teams.map(t=>Math.abs(t.goal_diff)))}),[teams]);
 ...
 return <div className="card lb-table-card">
  ...
  {loading?<div className="lb-loading">Loading opponents from Supabase…</div>:error?<div className="lb-loading">Unable to load: {error}</div>:<div className="lb-table-wrap"><table className="lb-table"><thead><tr><th className="col-rank">#</th><th className="col-team">Opponent</th><th className="col-stat">{sortableHeading('P','played')}</th><th className="col-stat">{sortableHeading('W','won')}</th><th className="col-stat">{sortableHeading('D','drawn')}</th><th className="col-stat">{sortableHeading('L','lost')}</th><th className="col-metric">{sortableHeading('GF','goals_for')}</th><th className="col-metric">{sortableHeading('GA','goals_against')}</th><th className="col-metric">{sortableHeading('GD','goal_diff')}</th><th className="col-metric">{sortableHeading('Win %','win_pct')}</th><th className="col-form">Last 5</th></tr></thead><tbody>
   {sorted.map((team,index)=><tr key={team.club_id} className={index===0&&sortKey==='played'&&sortDir==='desc'?'row-top':''}><td className="col-rank"><span className={`rank-badge ${index===0?'rank-top':index<3?'rank-podium':''}`}>{index+1}</span></td><td className="col-team"><div className="team-cell"><span className="club-badge opponent-badge">{team.crest_url?<img src={team.crest_url} alt=""/>:initials(team.opponent)}</span><span className="team-name">{team.opponent}</span></div></td><td className="col-stat">{team.played}</td><td className="col-stat">{team.won}</td><td className="col-stat">{team.drawn}</td><td className="col-stat">{team.lost}</td>
   <td className="col-metric"><div className="metric-cell"><span className="metric-value">{team.goals_for}</span><span className="metric-track"><span className="metric-fill orange-fill" style={{width:`${team.goals_for/max.gf*100}%`}}/></span></div></td>
   <td className="col-metric"><div className="metric-cell"><span className="metric-value">{team.goals_against}</span><span className="metric-track"><span className="metric-fill dark-fill" style={{width:`${team.goals_against/max.ga*100}%`}}/></span></div></td>
   <td className="col-metric"><div className="metric-cell"><span className="metric-value">{team.goal_diff>0?'+':''}{team.goal_diff}</span><span className="metric-track"><span className="metric-fill soft-fill" style={{width:`${Math.abs(team.goal_diff)/max.gd*100}%`}}/></span></div></td>
   ...
   <td className="col-form"><div className="form-row">{team.last5.split('').map((result,resultIndex)=><FormBadge key={resultIndex} result={result as 'W'|'D'|'L'}/>)}</div></td></tr>)}
  </tbody></table></div>}
 </div>;
}

...
type SortKey='played'|'won'|'drawn'|'lost'|'goals_for'|'goals_against'|'goal_diff'|'goals_per_game'|'clean_sheets'|'win_pct'|'loss_pct'|'last_meeting'|'last_won'|'last_lost';
type SortDir='asc'|'desc';
type FilterKey='All'|'League'|'Premier League'|'FA Cup'|'League Cup'|'Europe'|'Home'|'Away';
const filters:FilterKey[]=['All','League','Premier League','FA Cup','League Cup','Europe','Home','Away'];
...
export default function Leaderboard(){
 ...
 <div className="card lb-table-card">
  <div className="lb-table-header"><div className="lb-title-group"><span className="section-kicker">Team comparison</span><h2>Opponents Faced</h2><p>All-time Leeds United record against every competitive opponent</p></div><div className="lb-table-meta"><span><i className="legend-dot orange-dot"/> Most played</span><span><i className="legend-dot dark-dot"/> Historical opponent</span></div></div>
  <div className="lb-filterbar"><span className="lb-filter-label">Competition</span><div className="lb-filter-pills">{filters.map(item=><button key={item} className={`lb-filter-pill ${filter===item?'active':''}`} onClick={()=>setFilter(item)} aria-pressed={filter===item}>{item}</button>)}<button className={`lb-five-toggle ${fivePlus?'active':''}`} onClick={()=>setFivePlus(v=>!v)} aria-pressed={fivePlus}><span className="lb-toggle-track"><span className="lb-toggle-knob"/></span><span>+5 Matches</span></button></div><label className="lb-search"><Search size={14}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search opponent" aria-label="Search opponents"/></label></div>
  {loading?<div className="lb-loading">Loading {filter.toLowerCase()} opponents from the Lufcdatabase…</div>:error?<div className="lb-loading"><strong>Data connection error:</strong> {error}</div>:<div className="lb-table-wrap"><table className="lb-table"><thead><tr><th className="col-rank">#</th><th className="col-team">Opponent</th><th className="col-stat">{sortableHeading('P','played')}</th><th className="col-stat">{sortableHeading('W','won')}</th><th className="col-stat">{sortableHeading('D','drawn')}</th><th className="col-stat">{sortableHeading('L','lost')}</th><th className="col-metric">{sortableHeading('GF','goals_for')}</th><th className="col-metric">{sortableHeading('GA','goals_against')}</th><th className="col-metric">{sortableHeading('GD','goal_diff')}</th><th className="col-metric">{sortableHeading('GPG','goals_per_game')}</th><th className="col-metric">{sortableHeading('CS','clean_sheets')}</th><th className="col-metric">{sortableHeading('Win %','win_pct')}</th><th className="col-metric">{sortableHeading('Loss %','loss_pct')}</th><th className="col-year">{sortableHeading('Last Played','last_meeting')}</th><th className="col-year">{sortableHeading('Last Won','last_won')}</th><th className="col-year">{sortableHeading('Last Lost','last_lost')}</th><th className="col-form">Last 5</th></tr></thead><tbody>
 ...
 <div className="card lb-legend" aria-label="Table abbreviations"><div className="lb-legend-items"><span>P Played</span><span>W Won</span><span>D Drawn</span><span>L Lost</span><span>GF Goals For</span><span>GA Goals Against</span><span>GD Goal Difference</span><span>GPG Goals Per Game</span><span>CS Clean Sheets</span></div><div className="lb-legend-last5">Last 5 matches played by most recent from right to left</div></div>
 </>;
}

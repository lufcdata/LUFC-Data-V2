...
export default function ManagerPerformanceRadar({managerId}:{managerId:number}){
 ...
 const values=axes.map(a=>a.score),averageValues=[data.avg_attacking_score,data.avg_results_score,data.avg_defensive_score,data.avg_achievements_score,data.avg_discipline_score,data.avg_personnel_score,data.avg_longevity_score],rings=[20,40,60,80,100];
 return <section className="card manager-radar-card"><div className="manager-radar-head"><div><span className="section-kicker">Manager profile</span><h2>Performance Distribution</h2><p>Intentionally harsh Leeds-relative scoring: a larger polygon means a stronger profile.</p></div><div className="manager-radar-head-meta"><span className="manager-radar-scale">0–100</span><span className="manager-radar-average-key"><i/>Leeds manager average</span></div></div>
  <div className="manager-radar-layout"><svg className="manager-radar-svg" viewBox="0 0 410 330" role="img" aria-label="Manager performance distribution radar chart with Leeds manager average comparison">
   ...
   <polygon className="manager-radar-average-shape" points={polygon(averageValues)}/>
  ...
  </svg><div className="manager-radar-logic"><strong>Scoring logic</strong><p><b>Attacking</b> goals/game · <b>Results</b> win rate · <b>Defensive</b> lower goals conceded/game + clean sheets/game · <b>Achievements</b> honours/promotions with relegations heavily penalised and top-flight-title ceilings · <b>Discipline</b> red cards/game, reversed so fewer is better · <b>Personnel</b> players used ÷ seasons, reversed so fewer players per season is better · <b>Longevity</b> days in charge.</p><span>The dotted grey polygon is the arithmetic mean of every Leeds manager's seven final radar scores. Rate metrics remain stabilised against the Leeds managerial average for very short spells; the overall scale is compressed so elite scores are harder to reach.</span></div></div>
 </section>;
}

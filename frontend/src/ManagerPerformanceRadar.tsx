...
type RadarRow={manager_id:number;attacking_score:number;results_score:number;defensive_score:number;achievements_score:number;discipline_score:number;personnel_score:number;longevity_score:number;goals_per_game:number|string|null;win_rate:number|string|null;goals_against_per_game:number|string|null;clean_sheets_per_game:number|string|null;honours:number;promotions:number;relegations:number;red_cards:number;players_used:number;days_in_charge:number};
type Axis={label:string;score:number;raw:string};
const cx=270,cy=205,radius=128,labelRadius=174;
...
export default function ManagerPerformanceRadar({managerId}:{managerId:number}){
 ...
 const axes=useMemo<Axis[]>(()=>data?[
  ...
  {label:'Discipline',score:data.discipline_score,raw:`${data.red_cards} red cards`},
  {label:'Personnel',score:data.personnel_score,raw:`${data.players_used} players used`},
 ...
 return <section className="card manager-radar-card"><div className="manager-radar-head"><div><span className="section-kicker">Manager profile</span><h2>Performance Distribution</h2><p>Scores are relative to Leeds managers: the larger the polygon, the stronger the profile.</p></div><span className="manager-radar-scale">0–100</span></div>
  <div className="manager-radar-layout"><svg className="manager-radar-svg" viewBox="0 0 540 420" role="img" aria-label="Manager performance distribution radar chart">
   ...
   {axes.map((a,i)=>{const p=point(i,100,axes.length,labelRadius),anchor=Math.abs(p.x-cx)<18?'middle':p.x<cx?'end':'start';return <g key={a.label} className="manager-radar-label" transform={`translate(${p.x} ${p.y})`}><text textAnchor={anchor} y="-8" className="manager-radar-score">{a.score}</text><text textAnchor={anchor} y="9" className="manager-radar-name">{a.label}</text><text textAnchor={anchor} y="25" className="manager-radar-raw">{a.raw}</text></g>})}
  </svg><div className="manager-radar-logic"><strong>How it is scored</strong><p><b>Attacking</b> goals/game · <b>Results</b> win rate · <b>Defensive</b> combines lower goals conceded/game with clean sheets/game · <b>Achievements</b> rewards honours and promotions, with relegations carrying a major penalty · <b>Discipline</b> rewards fewer red cards · <b>Personnel</b> players used · <b>Longevity</b> days in charge.</p><span>Rate metrics are stabilised against the Leeds managerial average before percentile scoring, reducing distortion from very short caretaker spells.</span></div></div>
 </section>;
}

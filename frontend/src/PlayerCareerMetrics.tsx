...
function rankFor(rows:LeaderboardRow[],playerId:number,key:keyof Pick<LeaderboardRow,'appearances'|'starts'|'sub_apps'|'sub_off'|'won'|'win_pct'|'goals'|'gpg'|'captain'|'red_cards'|'penalties_taken'|'penalties_scored'|'penalties_missed'|'penalty_conversion_pct'>){const current=rows.find(r=>r.player_id===playerId);if(!current)return undefined;const value=Number(current[key]);return ordinal(1+rows.filter(r=>Number(r[key])>value).length)}
...
export default function PlayerCareerMetrics({playerId}:{playerId:number}){
 ...
 useEffect(()=>{let cancelled=false;(async()=>{if(!supabase)return;
  ...
  const next:Metric[]=[
   ...
   {label:'Penalty Conversion',value:pens?`${penPct.toFixed(1)}%`:'—',fill:pens?penPct:0,rank:pens?rankFor(rows.filter(r=>Number(r.penalties_taken)>0),playerId,'penalty_conversion_pct'):undefined},
